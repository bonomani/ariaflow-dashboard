...
export function badgeClass(status: string | null | undefined): string {
  if (['done', 'converged', 'ok', 'complete'].includes(status as string)) return 'badge good';
  if (['error', 'failed', 'missing', 'stopped'].includes(status as string)) return 'badge bad';
  if (
    ['paused', 'queued', 'waiting', 'unchanged', 'skipped', 'cancelled'].includes(status as string)
  ) {
    return 'badge warn';
  }
  return 'badge';
