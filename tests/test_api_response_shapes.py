...
SHAPE_CASES = [
    ...
    ("api-sessions-stats.schema.json", DEFAULT_SESSIONS_STATS, "DEFAULT_SESSIONS_STATS"),
    ("api-torrents.schema.json",       DEFAULT_TORRENTS,       "DEFAULT_TORRENTS"),
    ("api-peers.schema.json",          DEFAULT_PEERS,          "DEFAULT_PEERS"),
    ("api-aria2-get-option.schema.json",        DEFAULT_ARIA2_GET_OPTION,        "DEFAULT_ARIA2_GET_OPTION"),
    ("api-aria2-get-global-option.schema.json", DEFAULT_ARIA2_GET_GLOBAL_OPTION, "DEFAULT_ARIA2_GET_GLOBAL_OPTION"),
    ("api-aria2-option-tiers.schema.json",      DEFAULT_ARIA2_OPTION_TIERS,      "DEFAULT_ARIA2_OPTION_TIERS"),
    ("api-downloads-archive.schema.json",       DEFAULT_DOWNLOADS_ARCHIVE,       "DEFAULT_DOWNLOADS_ARCHIVE"),
    ("api-health.schema.json",                  DEFAULT_HEALTH,                  "DEFAULT_HEALTH"),
    ("api-bandwidth-probe.schema.json",         DEFAULT_BANDWIDTH_PROBE,         "DEFAULT_BANDWIDTH_PROBE"),
]
...
@pytest.mark.parametrize("schema_name,mock,label", SHAPE_CASES, ids=[c[0] for c in SHAPE_CASES])
...
def test_fake_backend_status_matches_schema() -> None:
    ...
    backend = FakeBackend()
    backend.add_items([{"url": "https://example.com/a.iso"}, {"url": "https://example.com/b.iso"}])
    ...
    assert not errors, "FakeBackend.status() does not match api-status.schema.json:\n" + "\n".join(
        f"  - {list(e.path)}: {e.message}" for e in errors
    )
