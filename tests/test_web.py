...
class WebSmokeTests(unittest.TestCase):
    def test_local_web_server_smoke(self) -> None:
        with patch(
            ...
            try:
                ...
                lifecycle_page = (
                    urllib.request.urlopen("http://127.0.0.1:8765/lifecycle", timeout=5)
                    .read()
                    .decode("utf-8")
                )
                ...
                log_page = (
                    urllib.request.urlopen("http://127.0.0.1:8765/log", timeout=5)
                    .read()
                    .decode("utf-8")
                )
                ...
                resp = urllib.request.urlopen(
                    "http://127.0.0.1:8765/api/discovery", timeout=5
                )
                ...
                try:
                    urllib.request.urlopen(
                        "http://127.0.0.1:8765/api/status", timeout=5
                    )
    ...
    def test_backend_url_injection(self) -> None:
        with patch(
            ...
            server = serve(
                host="127.0.0.1", port=8766, backend_url="http://custom:9999"
            )
            thread = threading.Thread(target=server.serve_forever, daemon=True)
            thread.start()
            time.sleep(0.2)
            try:
                page = (
                    urllib.request.urlopen("http://127.0.0.1:8766/", timeout=5)
                    .read()
                    .decode("utf-8")
                )
