...
class TestDashboardButtons:
    ...
    def test_stop_button_not_rendered(self, page: Page, web_server: str) -> None:
        _wait_dashboard(page, web_server)
        assert (
            page.locator('.panel .gap-sm.mb-md > button:has-text("Stop")').count() == 0
        )
    ...
    def test_new_session_button_not_rendered(self, page: Page, web_server: str) -> None:
        ...
        assert (
            page.locator(
                '.panel .gap-sm.mb-md > button:has-text("New Session")'
            ).count()
            == 0
        )

    def test_archive_button_stays_rendered_without_archiveable_items(
    ...
    def test_archive_button_uses_complete_and_failed_counts(
        self, page: Page, web_server: str
    ) -> None:
        _wait_dashboard(page, web_server)
        page.evaluate("""(() => {
    ...
    def test_primary_filter_buttons_stay_visible(
        self, page: Page, web_server: str
    ) -> None:
        _wait_dashboard(page, web_server)
    ...
    def test_pause_state_does_not_change_current_filter(
        self, page: Page, web_server: str
    ) -> None:
        _wait_dashboard(page, web_server)
    ...
    def test_add_backend_button(self, page: Page, web_server: str) -> None:
        ...
        backends = page.evaluate(
            "document.querySelector('[x-data]')._x_dataStack[0].loadBackendState().backends"
        )
...
class TestFilterButtons:
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                "Downloading",
                1,
                marks=pytest.mark.xfail(reason="flaky filter visibility"),
            ),
            pytest.param(
                "Paused", 1, marks=pytest.mark.xfail(reason="flaky filter visibility")
            ),
            ("Done", 1),
            ("Error", 1),
        ],
    ...
    def test_filter_button(
        self, page: Page, web_server: str, filter_name: str, expected_count: int
    ) -> None:
        _wait_dashboard(page, web_server)
        page.wait_for_timeout(500)
        ...
        if btn.count() == 0:
            pytest.skip(
                f"Filter button '{filter_name}' not visible (no items with that status)"
            )
        btn.first.click()
        page.wait_for_timeout(500)
        assert (
            len(page.query_selector_all(".item.compact:not(.add-card)"))
            == expected_count
        )
...
class TestItemActionButtons:
    def test_pause_button_on_downloading_item(
        self, page: Page, web_server: str
    ) -> None:
        _wait_dashboard(page, web_server)
    ...
    def test_remove_button_exists_on_every_item(
        self, page: Page, web_server: str
    ) -> None:
        _wait_dashboard(page, web_server)
        remove_btns = page.locator(
            '.item.compact:not(.add-card) button:has-text("Remove")'
        ).all()
    ...
    def test_no_pause_button_on_done_item(self, page: Page, web_server: str) -> None:
        ...
        visible = page.evaluate(
            """Array.from(document.querySelectorAll('.item.compact:not(.add-card) button')).filter(b => b.textContent.trim() === 'Pause').filter(b => getComputedStyle(b).display !== 'none').length"""
        )
        assert visible == 0
    ...
    def test_remove_button_on_queued_item(self, page: Page, web_server: str) -> None:
        ...
        page.wait_for_timeout(300)
        visible = page.evaluate(
            """Array.from(document.querySelectorAll('.item.compact:not(.add-card) button')).filter(b => b.textContent.trim() === 'Remove').filter(b => getComputedStyle(b).display !== 'none').length"""
        )
        assert visible >= 1
    ...
    def test_no_retry_button_on_queued_item(self, page: Page, web_server: str) -> None:
        ...
        page.wait_for_timeout(300)
        visible = page.evaluate(
            """Array.from(document.querySelectorAll('.item.compact:not(.add-card) button')).filter(b => b.textContent.trim() === 'Retry').filter(b => getComputedStyle(b).display !== 'none').length"""
        )
        assert visible == 0
...
class TestLogButtons:
    ...
    def test_load_declaration_button(self, page: Page, web_server: str) -> None:
        ...
        page.wait_for_timeout(500)
        page.evaluate(
            "document.querySelector('[x-data]')._x_dataStack[0].declarationText = ''"
        )
    ...
    def test_save_declaration_button(self, page: Page, web_server: str) -> None:
        ...
        page.wait_for_timeout(500)
        page.evaluate(
            """document.querySelector('[x-data]')._x_dataStack[0].declarationText = JSON.stringify({"uic": {}, "ucc": {}, "policy": {}})"""
        )
    ...
    @pytest.mark.parametrize(
        "model_name", ["actionFilter", "targetFilter", "sessionFilter"]
    )
    def test_log_filter_dropdown(
        self, page: Page, web_server: str, model_name: str
    ) -> None:
        _goto(page, f"{web_server}/log")
        page.wait_for_selector("body.page-log", timeout=8000)
        page.wait_for_timeout(300)
...
class TestBandwidthConfigButtons:
    ...
    def _bw_input_by_label(
        self, page: Page, web_server: str, label_text: str
    ) -> object:
...
class TestRefreshControl:
    def test_refresh_interval_change(self, page: Page, web_server: str) -> None:
        ...
        page.wait_for_timeout(300)
        val = page.evaluate(
            "document.querySelector('[x-data]')._x_dataStack[0].refreshInterval"
        )
